...
# metadata.openclaw 在 16 个 skill 上全都有，拿它判定会把 reasonix / generic 项目误认成 OpenClaw。
...
echo "$stale_previous_out" | grep -q '低于 v31' || fail "session-start did not warn for agents_version 30 stale v31 deployment"
...
echo "$newer_project_out" | grep -q '高于本 hook 支持的 v31' || fail "session-start did not reject agents_version 32 downgrade"
...
if echo "$mixed_version_out" | grep -q '低于 v31'; then
  fail "session-start incorrectly nagged '低于 v31' for current agents_version=31 just because setup_skill_version lags"
fi
if echo "$mixed_version_out" | grep -q '高于本 hook'; then
  fail "session-start incorrectly nagged '高于本 hook' for current agents_version=31 just because setup_skill_version lags"
fi
...
assert_grep 'AGENTS_VERSION.*-lt 31|AGENTS_VERSION" -lt 31' "$HOOKS_DIR/session-start.sh" "session-start must warn for stale agents_version under v30 deployment"
assert_grep 'AGENTS_VERSION.*-gt 31|AGENTS_VERSION" -gt 31' "$HOOKS_DIR/session-start.sh" "session-start must reject a newer agents_version as a downgrade"
...
assert_grep 'agents_version.*小于 `31`|版本 < 31' "$SKILL_DIR/SKILL.md" "story-setup redeploy branch must treat agents_version 29 as stale"
